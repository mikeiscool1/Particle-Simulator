...
impl Component for Grid {
    fn draw(&self, state: &State) {
        ...
        for i in 0..=steps {
            ...
            let xz_color = Color::new(
                0.5,
                0.5,
                0.5,
                fade((vec3(0.0, 0.0, oz + t) - camera_pos).length()) * 0.8 + 0.1,
            );
            let xy_color = Color::new(
                0.4,
                0.4,
                0.5,
                fade((vec3(0.0, oy + t, 0.0) - camera_pos).length()) * 0.7 + 0.1,
            );
            let yz_color = Color::new(
                0.5,
                0.4,
                0.4,
                fade((vec3(ox + t, 0.0, 0.0) - camera_pos).length()) * 0.7 + 0.1,
            );

            push_line(
                vec3(ox - grid_size, 0.0, oz + t),
                vec3(ox + grid_size, 0.0, oz + t),
                xz_color,
                &mut vertices,
                &mut indices,
